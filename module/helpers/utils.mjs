...
export async function doCombatRoll(actor, roll, label, critInfo) {
    ...
    var baseDieResultWithNatMod = Math.max(Math.min(baseDieResult + naturalResultMod, 10), 1);
    ...
    const totalRollAutomated = game.settings.get('dragon-ball-universe', 'automateNatMod');
    ...
    if (isCrit && critAutomated) {
        ...
        var criticalDiceRoll = new Roll(`${diceTotal}+${criticalDice}`, actor.getRollData());
    ...
    if (isBotch && botchAutomated) {
        ...
        var botchDiceRoll = new Roll(`${diceTotal}-${actor.system.baseTierOfPower * 2}`, actor.getRollData());
...
export function prepareRollData(actorSystem, combatRoll) {
    var topExtraDiceCategory = actorSystem.combatRollsExtraDiceCategory.tierOfPower[combatRoll];
    var greaterDiceCategory = actorSystem.combatRollsExtraDiceCategory.greaterDice[combatRoll];
    var criticalDiceCategory = actorSystem.combatRollsExtraDiceCategory.criticalDice[combatRoll];
    ...
    return { topExtraDiceCategory, topExtraDiceAmount, greaterDiceCategory, greaterDiceAmount, criticalDiceCategory, criticalDiceAmount, extraDice };
}
...
export async function prepareCombatRollPrompt(combatRoll, woundType = '') {
    ...
    var topBonusInput = fields.createTextInput({
        name: 'topBonus',
        value: 0
    });

    var topBonusGroup = fields.createFormGroup({
        input: topBonusInput,
        label: "Any (T) modifiers?"
    });

    var bTopBonusInput = fields.createTextInput({
        name: 'bTopBonus',
        value: 0
    });

    var bTopBonusGroup = fields.createFormGroup({
        input: bTopBonusInput,
        label: "Any (bT) modifiers?"
    });

    content += `<section class='grid grid-2col'> <div>${flatBonusGroup.outerHTML}<br> ${topBonusGroup.outerHTML}<br> ${bTopBonusGroup.outerHTML}`
    ...
    content += ` <br><span data-tooltip='This applies ToP Dice X more times'>${topDiceAgainGroup.outerHTML}</span><br> <span data-tooltip='This applies Greater Dice X more times'>${greaterDiceAgainGroup.outerHTML}</span>`

    if (combatRoll == 'Wound' && !woundType.includes('magic')) content += `<br> <span data-tooltip='This applies Super Stack Dice X more times'>${superStackAgainGroup.outerHTML}</span>`
    ...
    content += ` <div>${extraD4Group.outerHTML} <br>${extraD6Group.outerHTML}<br> ${extraD8Group.outerHTML}<br> ${extraD10Group.outerHTML}<br><span data-tooltip='This will declare a Natural Increase or Decrease on the Base Die.'>${natModGroup.outerHTML}</span>`;

    if (combatRoll == 'Wound' && !woundType.includes('magic')) content += `<br> ${superStackCatGroup.outerHTML}`;
    ...
    content += ` <div>${topDiceCatGroup.outerHTML}<br>${greaterDiceCatGroup.outerHTML}<br>${criticalDiceCatGroup.outerHTML}</div>`;
